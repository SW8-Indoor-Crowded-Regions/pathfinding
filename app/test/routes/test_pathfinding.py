...
def test_pathfinding_returns_correct_path(load_mock_payload):
    ...
    assert data['fastest_path'] == [{'id': '67d935b1d6d3ce76bef2c8e9', 'longitude': 16.369509547137667, 'latitude': 23.8621854675587}, {'id': '67d935b1d6d3ce76bef2c8ea', 'longitude': 61.74727469899295, 'latitude': 72.87442522121299}]
...
def test_pathfinding_handles_incorrect_source_room(load_mock_payload):
    ...
    assert response.json()['detail'] == "Source room 'non_existent_room' not found in the sensor graph."
...
def test_pathfinding_handles_incorrect_target_room(load_mock_payload):
    ...
    assert response.status_code == 400
    assert response.json()['detail'] == "Target room 'non_existent_sensor' not found in the sensor graph."
